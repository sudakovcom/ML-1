...
class Boosting:
    ...
    def fit_new_base_model(self, x, y, predictions):
        n = x.shape[0]
        bootstrap_sample = np.random.choice(np.arange(n), size=int(n*self.subsample), replace=True)
        x_ = x[bootstrap_sample]
        y_ = y[bootstrap_sample]
        predictions_ = predictions[bootstrap_sample]

        loss_derivative = self.loss_derivative(y_, predictions_)
        model = self.base_model_class(**self.base_model_params)
        model.fit(x_, -loss_derivative)
    ...
    def fit(self, x_train, y_train, x_valid, y_valid):
        ...
        for i in range(self.n_estimators):
            ...
            if self.early_stopping_rounds is not None:
                ...
                if valid_loss < self.loss_fn(y_valid, valid_predictions):
                    break
    ...
    def predict_proba(self, x):
        ...
        for gamma, model in zip(self.gammas, self.models):
            prediction += model.predict(x) * gamma * self.learning_rate
        return np.hstack(((1 - self.sigmoid(prediction)).reshape(-1, 1), (self.sigmoid(prediction)).reshape(-1, 1)))
